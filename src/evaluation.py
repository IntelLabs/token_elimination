...
def get_score_result_dict(scores):
    results_dict = {}
    for score, score_obj in scores.items():
        for key in ["precision", "recall", "fmeasure"]:
            results_dict[f"{score}_{key}"] = getattr(score_obj, key)
    return results_dict


def get_rouge(sentences, scorer):
    scores = scorer.score(*[normalize_answer(s) for s in sentences])
    return get_score_result_dict(scores)


def get_max_rouge(prediction, ground_truths, scorer):
    max_mean = -1
    max_metric = None
    for gt in ground_truths:
        result = get_rouge([prediction, gt], scorer)
        current_mean = np.mean([v for v in result.values()])
